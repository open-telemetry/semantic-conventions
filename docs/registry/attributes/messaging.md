...
- [IBM MQ Attributes](#ibm-mq-attributes)
...
This group describes attributes specific to IBM MQ.

**Attributes:**

| Key | Stability | Value Type | Description | Example Values |
| --- | --- | --- | --- | --- |
| <a id="messaging-ibmmq-queue-manager-id" href="#messaging-ibmmq-queue-manager-id">`messaging.ibmmq.queue_manager.id`</a> | ![Development](https://img.shields.io/badge/-development-blue) | string | The Queue Manager Identifier (QMID), a queue manager attribute generated by IBM MQ when the queue manager is created, formatted as `<QUEUE_MANAGER_NAME>_YYYY-MM-DD_HH.MM.SS`. The value reported MUST have the trailing padding removed. [10] | `QM1_2026-08-10_23.23.00` |

**[10] `messaging.ibmmq.queue_manager.id`:** Queue manager *names* are not globally unique in federated architectures, so the QMID is required to identify a queue manager unambiguously. The value is obtained from the `MQCA_Q_MGR_IDENTIFIER` queue manager attribute, which is stored as a fixed width MQI character field.
...
| Key | Stability | Value Type | Description | Example Values |
| --- | --- | --- | --- | --- |
| <a id="messaging-kafka-cluster-id" href="#messaging-kafka-cluster-id">`messaging.kafka.cluster.id`</a> | ![Development](https://img.shields.io/badge/-development-blue) | string | The Kafka cluster ID, obtained from the broker metadata exposed through the Kafka client (or AdminClient) API. [11] | `MkU3OEVBNTcwNTJENDM2Qk` |
