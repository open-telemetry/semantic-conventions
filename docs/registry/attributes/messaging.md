...
| Key | Stability | Value Type | Description | Example Values |
| --- | --- | --- | --- | --- |
| <a id="messaging-ibmmq-queue-manager-id" href="#messaging-ibmmq-queue-manager-id">`messaging.ibmmq.queue_manager.id`</a> | ![Development](https://img.shields.io/badge/-development-blue) | string | The intrinsic Queue Manager Identifier (QMID), a 48-byte queue manager attribute generated by IBM MQ when the queue manager is created. [10] | `QM1_2026-08-10_23.23.00` |

**[10] `messaging.ibmmq.queue_manager.id`:** Queue manager *names* are not globally unique in federated architectures, so the QMID is required to identify a queue manager unambiguously. Obtained from the `MQCA_Q_MGR_IDENTIFIER` queue manager attribute.
