...
| <a id="gcp-gce-instance-labels" href="#gcp-gce-instance-labels">`gcp.gce.instance.labels.<key>`</a> | string | GCE instance labels, `<key>` being the label name and the value being the label value. [6] | `prod` | ![Development](https://img.shields.io/badge/-development-blue) |
...
**[6] `gcp.gce.instance.labels.<key>`:** For example, a GCE instance label `env` with value `prod` SHOULD be recorded as the `gcp.gce.instance.labels.env` attribute with value `"prod"`. The `<key>` MUST be the exact GCE instance label key.
