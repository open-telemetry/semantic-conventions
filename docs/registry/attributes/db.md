...
| --- | --- | --- | --- | --- |
| <a id="db-client-connection-pool-name" href="#db-client-connection-pool-name">`db.client.connection.pool.name`</a> | ![Development](https://img.shields.io/badge/-development-blue) | string | The name of the connection pool; unique within the instrumented application. In case the connection pool implementation doesn't provide a name, instrumentation SHOULD use a combination of parameters that would make the name unique, for example, combining attributes `server.address` and `server.port`, formatted as `server.address:server.port`. Instrumentations that generate connection pool name following different patterns SHOULD document it. | `myDataSource` |
