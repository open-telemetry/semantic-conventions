...
| <a id="k8s-node-condition-reason" href="#k8s-node-condition-reason">`k8s.node.condition.reason`</a> | string | The reason for the condition's last transition [17] | `KubeletHasSufficientMemory`; `KubeletHasNoDiskPressure`; `KubeletHasSufficientPID` | ![Development](https://img.shields.io/badge/-development-blue) |
...
**[17] `k8s.node.condition.reason`:** This attribute aligns with the `reason` field of the
