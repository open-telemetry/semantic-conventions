...
| <a id="gen-ai-guardian-provider-name" href="#gen-ai-guardian-provider-name">`gen_ai.guardian.provider.name`</a> | ![Development](https://img.shields.io/badge/-development-blue) | string | The provider or vendor of the guardian service. | `aws.bedrock`; `azure.ai.content_safety`; `zenity`; `custom` |
...
**[3] `gen_ai.guardian.id`:** This is a vendor-neutral identifier for the security component that evaluated content or an action. It may map to provider-specific IDs such as `aws.bedrock.guardrail.id`.
