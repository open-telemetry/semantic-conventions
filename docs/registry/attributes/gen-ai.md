...
**[2] `gen_ai.evaluation.score.label`:** This attribute captures the categorical result of an evaluation. It MAY be used alongside `score.value`, or independently when no numeric score is produced. The label SHOULD have low cardinality. Possible values depend on the evaluation metric and evaluator used; implementations SHOULD document the possible values.
