...
| <a id="gen-ai-evaluation-name" href="#gen-ai-evaluation-name">`gen_ai.evaluation.name`</a> | string | The name of the evaluation used for the GenAI response. | `Relevance`; `IntentResolution` | ![Development](https://img.shields.io/badge/-development-blue) |
...
| <a id="gen-ai-metadata" href="#gen-ai-metadata">`gen_ai.metadata`</a> | string | Metadata associated with Gen AI operation. [2] | `{\"gen_ai.thread.id\": \"thread_ggguJ0iZXRPjUnCy9vT9Fdvs\"}` | ![Development](https://img.shields.io/badge/-development-blue) |
...
**[2] `gen_ai.metadata`:** The structure is specific to Gen AI operatiption. If the metadata is structured, it is RECOMMENDED to provide it in a structured form using language-specific API. It can also be captured as a JSON string when structured API is not available.
