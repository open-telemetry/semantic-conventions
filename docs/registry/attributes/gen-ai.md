...
|---|---|---|---|---|
| <a id="gen-ai-agent-child-agents" href="#gen-ai-agent-child-agents">`gen_ai.agent.child_agents`</a> | any | Information about child agents utilized during the agent invocation. [1] | `[
  {
    "agent_id": "agent_id_1",
    "name": "WeatherAgent",
    "role": "specialist"
...
| <a id="gen-ai-agent-invocation-input" href="#gen-ai-agent-invocation-input">`gen_ai.agent.invocation_input`</a> | any | The input messages provided to the agent invocation. [2] | `[
  {
    "role": "system",
    "body": [
      {
...
| <a id="gen-ai-agent-invocation-output" href="#gen-ai-agent-invocation-output">`gen_ai.agent.invocation_output`</a> | any | The output messages generated by the agent invocation. [3] | `[
  {
    "role": "assistant",
    "body": [
      {
...
| <a id="gen-ai-data-source-id" href="#gen-ai-data-source-id">`gen_ai.data_source.id`</a> | string | The data source identifier. [4] | `H7STPQYOND` | ![Development](https://img.shields.io/badge/-development-blue) |
...
| <a id="gen-ai-tool-call-arguments" href="#gen-ai-tool-call-arguments">`gen_ai.tool.call.arguments`</a> | any | Parameters passed to the tool call. [9] | `{"location": "San Francisco?", "date": "2025-10-01"}` | ![Development](https://img.shields.io/badge/-development-blue) |
...
| <a id="gen-ai-tool-call-result" href="#gen-ai-tool-call-result">`gen_ai.tool.call.result`</a> | any | The result returned by the tool call (if any and if execution was successful). [10] | `{"temperature_range": {"high": 75, "low": 60}, "conditions": "sunny"}` | ![Development](https://img.shields.io/badge/-development-blue) |
| <a id="gen-ai-tool-definitions" href="#gen-ai-tool-definitions">`gen_ai.tool.definitions`</a> | any | The list of tool definitions available to the GenAI agent or model. [11] | `[{"type":"function","name":"get_current_weather","description":"Get the current weather in a given location","parameters":{"type":"object","properties":{"location":{"type":"string","description":"The city and state, e.g. San Francisco, CA"},"unit":{"type":"string","enum":["celsius","fahrenheit"]}},"required":["location","unit"]}}]` | ![Development](https://img.shields.io/badge/-development-blue) |
...
**[1] `gen_ai.agent.child_agents`:** This attribute contains an array of objects representing sub-agents or specialized 
agents that were invoked as part of the main agent's execution flow.

The structure is an array where each object contains:
- `agent_id`: Unique identifier for the child agent
- `name`: Human-readable name of the child agent
- `role`: Classification of the child agent (e.g., "specialist", "coordinator", "validator")

This enables tracking of agent orchestration patterns and multi-agent workflows.
...
**[2] `gen_ai.agent.invocation_input`:** This attribute contains an array of structured message objects representing the conversation
context and input provided to the agent. Each message includes role and body information.

The structure follows a consistent format:
- Each message has a `role` field (system, user, assistant, tool)
- Each message has a `body` array containing content objects
- Content objects specify `type` (typically "text") and `content` with the actual message
...
**[3] `gen_ai.agent.invocation_output`:** This attribute contains an array of structured message objects representing the agent's
response and output. Each message includes role, body content, and finish reason.

The structure follows a consistent format:
- Each message has a `role` field (typically "assistant" for agent responses)
- Each message has a `body` array containing content objects
- Content objects specify `type` (typically "text") and `content` with the actual response
- Messages may include a `finish_reason` indicating why generation stopped
...
**[9] `gen_ai.tool.call.arguments`:** TODO: we need to decide what to do with well-known function call params (eg. filesearch) - do we want to define a format and normalize across models/frameworks or leave them as is and record a complex object.
...
**[10] `gen_ai.tool.call.result`:** TODO: we need to decide what to do with well-known function call results
...
**[11] `gen_ai.tool.definitions`:** The value of this attribute matches provider-specific tool definition format. It's expected to be an object - in case a serialized string is available to the instrumentation, the instrumentation SHOULD do the best effort to deserialize it to an object.
