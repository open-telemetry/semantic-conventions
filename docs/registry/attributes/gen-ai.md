...
| <a id="gen-ai-evaluation-label" href="#gen-ai-evaluation-label">`gen_ai.evaluation.label`</a> | string | Human readable label for evaluation. | `relevant`; `not_relevant`; `correct`; `incorrect`; `pass`; `fail` | ![Development](https://img.shields.io/badge/-development-blue) |
| <a id="gen-ai-evaluation-name" href="#gen-ai-evaluation-name">`gen_ai.evaluation.name`</a> | string | The name of the evaluation used for the GenAI response. [2] | `Relevance`; `IntentResolution` | ![Development](https://img.shields.io/badge/-development-blue) |
...
| <a id="gen-ai-evaluation-score" href="#gen-ai-evaluation-score">`gen_ai.evaluation.score`</a> | double | The evaluation score returned by the evaluator. | `4.0` | ![Development](https://img.shields.io/badge/-development-blue) |
| <a id="gen-ai-input-metadata" href="#gen-ai-input-metadata">`gen_ai.input.metadata`</a> | string | Metadata associated with the GenAI input. [3] | `{\"requestId\": \"fab3ee5d-a3c6-4c47-b3de-901bf02fa045\"}` | ![Development](https://img.shields.io/badge/-development-blue) |
| <a id="gen-ai-metadata" href="#gen-ai-metadata">`gen_ai.metadata`</a> | string | Metadata associated with Gen AI operation. [4] | `{\"gen_ai.thread.id\": \"thread_ggguJ0iZXRPjUnCy9vT9Fdvs\"}` | ![Development](https://img.shields.io/badge/-development-blue) |
...
| <a id="gen-ai-output-metadata" href="#gen-ai-output-metadata">`gen_ai.output.metadata`</a> | string | Metadata associated with the GenAI output. [6] | `{\"Perplexity\": 1.335}` | ![Development](https://img.shields.io/badge/-development-blue) |
...
| <a id="gen-ai-provider-name" href="#gen-ai-provider-name">`gen_ai.provider.name`</a> | string | The Generative AI provider as identified by the client or server instrumentation. [8] | `openai`; `gcp.gen_ai`; `gcp.vertex_ai` | ![Development](https://img.shields.io/badge/-development-blue) |
...
| <a id="gen-ai-request-encoding-formats" href="#gen-ai-request-encoding-formats">`gen_ai.request.encoding_formats`</a> | string[] | The encoding formats requested in an embeddings operation, if specified. [9] | `["base64"]`; `["float", "binary"]` | ![Development](https://img.shields.io/badge/-development-blue) |
...
**[2] `gen_ai.evaluation.name`:** This attribute identifies the evaluation name.
...
**[4] `gen_ai.metadata`:** The structure is specific to Gen AI operatiption. If the metadata is structured, it is RECOMMENDED to provide it in a structured form using language-specific API. It can also be captured as a JSON string when structured API is not available.
...
**[6] `gen_ai.output.metadata`:** The structure is specific to the implementation. If the metadata is structured, it is RECOMMENDED to provide it in a structured form using language-specific API. It can also be captured as a JSON string when structured API is not available. If metadata properties contain any sensitive information such as prompts or completions, corresponding properties MUST NOT be recorded by default. Instrumentations MAY provide a way to override this behavior and record sensitive information in the metadata if user explicitly allows it.
