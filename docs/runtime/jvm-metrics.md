...
In case the limit is not available or not set, the returned value may be negative or equal to `Long.MAX_VALUE` (`2^63-1`), such values must be filtered out by producers of this metric.
