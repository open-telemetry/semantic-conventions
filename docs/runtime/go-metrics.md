...
As of Go 1.26, the `go.memory.detailed_type` attribute has the following relationship to the `go.memory.type` attributes. Values for `go.memory.detailed_type` are subject to change with the Go version used:
...
As of Go 1.26, the `go.cpu.detailed_state` attribute has the following relationship to the `go.cpu.state` attributes. Values for `go.cpu.detailed_state` are subject to change with the Go version used:
