...
This event SHOULD be emitted by an SDK provider instance
(`TracerProvider`, `LoggerProvider`, `MeterProvider`) when its shutdown
operation completes, and MAY be emitted by any other SDK component
(for example a processor or exporter) when it shuts down. The component
that shut down is identified by `otel.component.type` and
`otel.component.name`.

When a component shuts down child components (as a provider shuts down
its processors and exporters), its event covers the whole operation,
including the time spent shutting those children down. If those children
also emit this event, the parent and child events describe nested,
overlapping shutdown intervals; this is expected.

When a process terminates without a component's shutdown being invoked
...
ended, whether it completed successfully, failed, or timed out. When
`otel.component.shutdown.duration` is present, the start of the shutdown
attempt can be reconstructed as `timestamp - duration`.
