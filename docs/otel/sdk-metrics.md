...
| Name | Instrument Type | Unit (UCUM) | Description | Stability | Entity Associations |
| -------- | --------------- | ----------- | -------------- | --------- | ------ |
| `otel.sdk.log.created` | Counter | `{log_record}` | The number of log records submitted to a `Logger`. [1] | ![Development](https://img.shields.io/badge/-development-blue) | |

**[1]:** This metric counts every log record submitted to a `Logger` via its `Emit` operation.
The record is counted at the point of submission, before any SDK-internal processing,
making this metric the top of the log delivery funnel.

The count is independent of the `Logger`'s enabled state and configuration. A record is
counted even if it is subsequently dropped or filtered within the SDK, for example because
the `Logger` is disabled, because of `LoggerConfig` rules such as `minimum_severity` or
`trace_based`, or because a processor or the export pipeline drops it. Each such drop is
therefore observable as a gap between this metric and the corresponding downstream metric
(e.g. `otel.sdk.processor.log.processed`).

Counting records submitted to a disabled `Logger`, or to a `LoggerProvider` with no
configured processor, surfaces these misconfigurations, which would otherwise be
indistinguishable from an application that is not producing logs.

Records that are never submitted to the SDK are not counted. In particular, a record that
instrumentation skips because a `Logger.Enabled()` check returned `false` is not counted,
since `Emit` is never invoked. Per the Logs API, `Enabled()` only suppresses records that
would otherwise be dropped, so this does not hide export-worthy records.
...
This metric SHOULD be specified with [`ExplicitBucketBoundaries` advisory parameter](https://github.com/open-telemetry/opentelemetry-specification/blob/v1.60.0/specification/metrics/api.md#instrument-advisory-parameters) with a single bucket with no boundaries.
