...
a disabled `Logger` is a No-op: emitting to it has no effect, so its records are not counted (this metric only counts records submitted to an enabled `Logger`).
