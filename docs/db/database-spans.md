...
Database spans MUST follow the overall [guidelines for span names](https://github.com/open-telemetry/opentelemetry-specification/blob/v1.57.0/specification/trace/api.md#span).
...
**[12] `db.operation.batch.size`:** A batch operation contains two or more database operations explicitly submitted
...
In batch APIs that execute the same parameterized operation with multiple parameter
