...
**[9] `server.port`:** If `server.address` is set and the client is configured with one or more database server endpoints that all use the same non-default port and the port is not included in `server.address`.
...
**[18] `server.address`:** Instrumentation SHOULD set `server.address` from the configuration used to create the database client
