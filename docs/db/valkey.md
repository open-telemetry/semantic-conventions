...
# Semantic conventions for Valkey client operations

**Status**: [Development][DocumentStatus]

The Semantic Conventions for [Valkey](https://valkey.io/) extend and override the [Database Semantic Conventions](database-spans.md).

## Spans

<!-- semconv span.db.valkey.client -->
...
Spans representing calls to Valkey adhere to the general [Semantic Conventions for Database Client Spans](/docs/db/database-spans.md).

`db.system.name` MUST be set to `"valkey"` and SHOULD be provided **at span creation time**.
...
| --- | --- | --- | --- | --- | --- |
| [`db.operation.name`](/docs/registry/attributes/db.md) | ![Stable](https://img.shields.io/badge/-stable-lightgreen) | `Required` | string | The Valkey command name. [1] | `HMSET`; `GET`; `SET` |
| [`db.namespace`](/docs/registry/attributes/db.md) | ![Stable](https://img.shields.io/badge/-stable-lightgreen) | `Conditionally Required` If and only if it can be captured reliably. | string | The [database index](https://valkey.io/commands/select/) associated with the connection, represented as a string. [2] | `0`; `1`; `15` |
| [`db.response.status_code`](/docs/registry/attributes/db.md) | ![Stable](https://img.shields.io/badge/-stable-lightgreen) | `Conditionally Required` [3] | string | The Valkey [simple error](https://valkey.io/topics/protocol/#simple-errors) prefix. [4] | `ERR`; `WRONGTYPE`; `CLUSTERDOWN` |
...
| [`db.query.text`](/docs/registry/attributes/db.md) | ![Stable](https://img.shields.io/badge/-stable-lightgreen) | `Recommended` | string | The full syntax of the Valkey CLI command. [9] | `HMSET myhash field1 ? field2 ?` |
...
For [transactions and pipelined calls](https://valkey.io/topics/transactions/), if the individual operations are known to have the same command then that command SHOULD be used prepended by `MULTI ` or `PIPELINE `. Otherwise `db.operation.name` SHOULD be `MULTI` or `PIPELINE`.
...
**[4] `db.response.status_code`:** All Valkey error prefixes SHOULD be considered errors.
...
The value provided for `db.query.text` SHOULD correspond to the syntax of the Valkey CLI. If, for example, the [`HMSET` command](https://valkey.io/commands/hmset/) is invoked, `"HMSET myhash field1 ? field2 ?"` would be a suitable value for `db.query.text`.

**[10] `db.stored_procedure.name`:** See [FCALL](https://valkey.io/commands/fcall/) and [EVALSHA](https://valkey.io/commands/evalsha/).
...
In this example, Valkey is connected using a UNIX domain socket and therefore the connection string is left out.

| Key                    | Value                                          |
| :--------------------- | :--------------------------------------------- |
| Span name              | `"HMSET"`                                      |
| `db.system.name`       | `"valkey"`                                     |
| `network.peer.address` | `"/tmp/valkey.sock"`                           |
