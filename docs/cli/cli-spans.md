...
| [`process.executable.path`](/docs/registry/attributes/process.md) | ![Deprecated](https://img.shields.io/badge/-deprecated-red)<br>`process.launch_path` should be used instead to capture the path used to launch the process. | `Recommended` | string | The full path to the process executable. On Linux based systems, can be set to the target of `/proc/[pid]/exe`. On Windows, can be set to the result of `GetProcessImageFileNameW`. | `/usr/bin/cmd/otelcol` |
