...
**[2]:** If readily available. Otherwise, when the instrumentation library supports parsing `db.statement` and it's enabled, `db.operation.name` SHOULD be set to the first operation name found in the query.
