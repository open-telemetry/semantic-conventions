...
**[8] `azure.resource.namespace`:** When `azure.resource.namespace` attribute is populated, it MUST be set to `Microsoft.DocumentDB` for all operations performed by Cosmos DB client.
...
| `db.system`                          | `"cosmosdb"` |
>>>>>>> 97d8d210 (Change azure_ to az. in enum values)
