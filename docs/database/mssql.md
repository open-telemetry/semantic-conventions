...
| [`db.collection.namespace`](../attributes-registry/db.md) | string | Identifies MS SQL database within a server. [3] | `instance1.products`; `customers` | `Conditionally Required` If applicable. | ![Experimental](https://img.shields.io/badge/-experimental-blue) |
...
**[3]:** When connecting to a default database, `db.collection.namespace` SHOULD be set to the name of the database. When connecting to a [named instance](https://learn.microsoft.com/sql/connect/jdbc/building-the-connection-url#named-and-multiple-sql-server-instances), `db.collection.namespace` SHOULD be set to the combination of instance and database name following the `{instance_name}.{database_name}` pattern.
