...
| Name     | Instrument Type | Unit (UCUM) | Description    | Stability | Entity Associations |
| -------- | --------------- | ----------- | -------------- | --------- | ------ |
| `k8s.container.ready` | UpDownCounter | `{container}` | Specifies whether the container is currently passing its readiness check, and is ready to accept traffic (0 for no, 1 for yes). [1] | ![Development](https://img.shields.io/badge/-development-blue) |  |

**[1]:** This metric aligns with the `ready` field of the
