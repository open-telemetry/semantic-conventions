...
**Description:** PersistentVolume level metrics captured under the namespace `k8s.persistentvolume`.
...
When the phase is `Bound`, the bound PVC name and namespace SHOULD be included as attributes.
...
| [`k8s.namespace.name`](/docs/registry/attributes/k8s.md) | ![Alpha](https://img.shields.io/badge/alpha-mediumpurple) | `Conditionally Required` When the PV is in the `Bound` phase. | string | The namespace of the bound PersistentVolumeClaim. | `default` |
...
**Description:** PersistentVolumeClaim level metrics captured under the namespace `k8s.persistentvolumeclaim`.
...
**[1]:** All possible phases should be reported at each interval to avoid gaps in the time series.
When the phase is `Bound`, the bound PV name SHOULD be included as an attribute.
This metric is derived from the `.status.phase` field of the
