...
It is computed by aggregating `cpu.time` across all logical CPUs over a measurement interval,
and dividing the result by the elapsed time and the `system.cpu.logical.count` to produce a normalized,
