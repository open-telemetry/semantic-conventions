...
| [`code.function.name`](/docs/attributes-registry/code.md) | string | The method or function fully-qualified name without arguments. The value should fit the natural representation of the language runtime, which is also likely the same used within `code.stacktrace` attribute value. | `com.example.MyHttpService.serveRequest (Java) com.mycompany.Main$1.myMethod (Java anynymous class) com.mycompany.Main$$Lambda/0x0000748ae4149c00.myMethod (Java lambda) GuzzleHttp\Client::transfer (PHP) github.com/my/repo/pkg.foo.func5 (Go) OpenTelemetry.Ctx.new (Elixir) opentelemetry_ctx:new (Erlang) playground::my_module::my_cool_func (Rust)` | `Recommended` | ![Development](https://img.shields.io/badge/-development-blue) |
