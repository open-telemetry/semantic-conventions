...
Attributes in the `server.service.*` and `client.service.*` namespaces describe the logical service
identity of the server and client sides of a connection. They enable observing both the upstream
client service and the downstream server service on the same signal.

Users can define what the name of a service is based on their particular semantics in their distributed system.

<!-- semconv remote.service -->
<!-- NOTE: THIS TEXT IS AUTOGENERATED. DO NOT EDIT BY HAND. -->
<!-- see templates/registry/markdown/snippet.md.j2 -->
<!-- prettier-ignore-start -->
