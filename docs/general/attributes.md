...
| `network.client.address` | string | Client address of the network connection - IP address or Unix domain socket name. | `/tmp/my.sock`; `10.1.2.80` | Recommended: If different than `client.address`. |
| `network.client.port` | int | Client port number of the network connection. | `65123` | Recommended: [2] |
| `network.server.address` | string | Server address of the network connection - domain name if available without reverse DNS lookup, otherwise IP address or Unix domain socket name. [3] | `example.com`; `10.1.2.80`; `/tmp/my.sock`; `proxy.example.com` | Recommended: If different than `server.address`. |
| `network.server.port` | int | Server port number of the network connection. | `65123` | Recommended: [4] |
| `network.server.ip` | string | Server IP address of the network connection. | `10.1.2.80` | Recommended: If different than `network.server.address`. |
