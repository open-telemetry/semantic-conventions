...
<a id="`destination.address`"></a>

| [`destination.address`](#`destination.address`) | string | Destination address - domain name if available without reverse DNS lookup; otherwise, IP address or Unix domain socket name. [1] | `destination.example.com`; `10.1.2.80`; `/tmp/my.sock` | ![Experimental](https://img.shields.io/badge/-experimental-blue) | |

### `destination.port`

<a id="`destination.port`"></a>

| [`destination.port`](#`destination.port`) | int | Destination port number | `3389`; `2888` | ![Experimental](https://img.shields.io/badge/-experimental-blue) | |
