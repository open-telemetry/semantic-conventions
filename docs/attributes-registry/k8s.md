...
| `k8s.pod.annotations.<key>` | string | The annotations placed on the Pod, the `<key>` being the annotation name, the value being the annotation value. | `k8s.pod.annotations.kubernetes.io/enforce-mountable-secrets=true`; `k8s.pod.annotations.mycompany.io/arch=x64`; `k8s.pod.annotations.data=` | Recommended |
