...
| <a id="code-line-number" href="#code-line-number">`code.line.number`</a> | int | The line number in `code.file.path` best representing the operation. It SHOULD point within the code unit named in `code.function.name`. When using OTLP Profiles this attribute is covered by 'message Line' and MUST NOT be used. This constraint is imposed to prevent redundancy and maintain data integrity. | `42` | ![Release Candidate](https://img.shields.io/badge/-rc-mediumorchid) |
