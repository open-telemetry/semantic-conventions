...
[logs](https://opentelemetry.io/docs/specs/otel/logs/api/#emit-a-logrecord)
emitted through the [Logger API](https://opentelemetry.io/docs/specs/otel/logs/api/#logger).
...
>   * When `OTEL_SEMCONV_EXCEPTION_SIGNAL_OPT_IN` is set to `logs` or `logs/dup`, but
>     exceptions cannot be emitted as logs because no `LoggerProvider` is configured,
>     the instrumentation MUST keep emitting exceptions as span events and MUST log
>     the following warning through the SDK internal logger:
>
>     ```text
>     The OTEL_SEMCONV_EXCEPTION_SIGNAL_OPT_IN environment variable is ignored because no logger provider is configured.
>     ```
...
[LogRecord](https://opentelemetry.io/docs/specs/otel/logs/data-model/#log-and-event-record-definition) passed to the [Logger](https://opentelemetry.io/docs/specs/otel/logs/api/#logger) emit
operations.
...
[Emit a LogRecord](https://opentelemetry.io/docs/specs/otel/logs/api/#emit-a-logrecord) API, instrumentations SHOULD provide the exception instance
...
[LogRecord](https://opentelemetry.io/docs/specs/otel/logs/data-model/#log-and-event-record-definition).
...
Describes a single exception and may be used on any signal that reports an exception.
