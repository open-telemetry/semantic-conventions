...
| [`network.server.address`](../general/attributes.md) | string | Server address of the network connection - domain name if available without reverse DNS lookup, otherwise IP address or Unix domain socket name. [17] | `example.com`; `10.1.2.80`; `/tmp/my.sock`; `proxy.example.com` | Recommended: If different than `server.address`. |
...
| [`network.server.port`](../general/attributes.md) | int | Server port number of the network connection. | `65123` | Recommended: [18] |
