...
| `messaging.message.body.size` | int | The size of the uncompressed message body in bytes. | `1439` | Recommended: [10] |
...
| `messaging.message.envelope.size` | int | The size of the uncompressed message body and metadata in bytes. | `2738` | Recommended: [12] |
| `messaging.message.id` | string | A value used by the messaging system as an identifier for the message, represented as a string. | `452a7c7c7c7048c2f887f61572b18fc2` | Recommended: [13] |
...
**[13]:** Only for spans that represent an operation on a single message.
