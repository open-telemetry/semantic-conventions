...
| [`messaging.destination.name`](/docs/registry/attributes/messaging.md) | string | The message destination name [1] | `MyQueue`; `MyTopic` | `Conditionally Required` [2] | ![Development](https://img.shields.io/badge/-development-blue) |
...
| [`error.type`](/docs/registry/attributes/error.md) | string | Describes a class of error the operation ended with. [6] | `amqp:decode-error`; `KAFKA_STORAGE_ERROR`; `channel-error` | `Conditionally Required` If and only if the messaging operation has failed. | ![Stable](https://img.shields.io/badge/-stable-lightgreen) |
| [`messaging.batch.message_count`](/docs/registry/attributes/messaging.md) | int | The number of messages sent, received, or processed in the scope of the batching operation. [7] | `0`; `1`; `2` | `Conditionally Required` [8] | ![Development](https://img.shields.io/badge/-development-blue) |
| [`messaging.rocketmq.message.delay_time_level`](/docs/registry/attributes/messaging.md) | int | The delay time level for delay message, which determines the message delay time. | `3` | `Conditionally Required` [9] | ![Development](https://img.shields.io/badge/-development-blue) |
| [`messaging.rocketmq.message.delivery_timestamp`](/docs/registry/attributes/messaging.md) | int | The timestamp in milliseconds that the delay message is expected to be delivered to consumer. | `1665987217045` | `Conditionally Required` [10] | ![Development](https://img.shields.io/badge/-development-blue) |
...
**[6] `error.type`:** The `error.type` SHOULD be predictable, and SHOULD have low cardinality.
...
**[8] `messaging.batch.message_count`:** If the span describes an operation on a batch of messages.

**[9] `messaging.rocketmq.message.delay_time_level`:** If the message type is delay and delivery timestamp is not specified.

**[10] `messaging.rocketmq.message.delivery_timestamp`:** If the message type is delay and delay time level is not specified.
