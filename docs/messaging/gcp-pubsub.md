...
- [Google Cloud Pub/Sub spans](#google-cloud-pubsub-spans)
...
- [Examples](#examples)
  - [Asynchronous batch publish example](#asynchronous-batch-publish-example)
  - [Unary pull example](#unary-pull-example)
...
Describes a message being created for publishing to Google Cloud Pub/Sub.
...
| [`messaging.system`](/docs/registry/attributes/messaging.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Required` | string | The messaging system as identified by the client instrumentation. [2] | `gcp_pubsub` [(see all values)](#messaging-system-values) |
| [`error.type`](/docs/registry/attributes/error.md) | ![Stable](https://img.shields.io/badge/-stable-lightgreen) | `Conditionally Required` If and only if the messaging operation has failed. | string | Describes a class of error the operation ended with. [3] | `NOT_FOUND`; `RESOURCE_EXHAUSTED`; `UNAVAILABLE` [(see all values)](#error-type-values) |
...
Describes a producer publishing one or more messages to Google Cloud Pub/Sub.
...
Describes a consumer receiving one or more messages from Google Cloud Pub/Sub (pull-based).
...
Describes a consumer processing one or more messages from Google Cloud Pub/Sub (push-based).
...
Describes settling (ack/nack/modack) one or more Google Cloud Pub/Sub messages.
...
- `ack` and `nack` for acknowledging or negatively acknowledging a message
