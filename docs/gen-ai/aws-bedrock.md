...
| [`gen_ai.request.stream`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Conditionally Required` if the request is a streaming request | boolean | Indicates whether the GenAI request was made in streaming mode. [8] | |
...
| [`gen_ai.response.time_to_first_chunk`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Recommended` [11] | double | Time in seconds to receive the first chunk in a streaming response. [12] | `0.5`; `1.2` |
...
**[8] `gen_ai.request.stream`:** When true, the response is delivered incrementally as it is generated. When false or not set, the response is delivered as a single complete response.
...
**[11] `gen_ai.response.time_to_first_chunk`:** if available and the request was a streaming request

**[12] `gen_ai.response.time_to_first_chunk`:** This attribute SHOULD only be set for streaming responses. The value is measured from when the client issues the generation request to when the first chunk is received in the response stream.
