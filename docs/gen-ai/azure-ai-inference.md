...
| [`gen_ai.request.logprobs`](/docs/registry/attributes/gen-ai.md) | boolean | Whether to return the log probabilities for each token in the response. [6] | `true` | `Conditionally Required` if available, in the request, and not false | ![Development](https://img.shields.io/badge/-development-blue) |
...
**[6] `gen_ai.request.logprobs`:** Whether to return the log probabilities for each token in the response. If the request includes this attribute, it MUST be set to `true`
