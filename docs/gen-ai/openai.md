...
| [`gen_ai.response.time_to_first_chunk`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Recommended` [12] | double | Time in seconds to receive the first chunk in a streaming response. [13] | `0.5`; `1.2` |
...
**[12] `gen_ai.response.time_to_first_chunk`:** if available and the request was a streaming request

**[13] `gen_ai.response.time_to_first_chunk`:** This attribute SHOULD only be set for streaming responses. The value is measured from when the client issues the generation request to when the first chunk is received in the response stream.
