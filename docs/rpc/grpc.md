...
| [`server.address`](/docs/registry/attributes/server.md) | ![Stable](https://img.shields.io/badge/-stable-lightgreen) | `Conditionally Required` [4] | string | The RPC server [host name](https://grpc.github.io/grpc/core/md_doc_naming.html) before name resolution, as provided in the target URL during gRPC channel creation. [5] | `example.com`; `10.1.2.80`; `/tmp/my.sock` |
| [`url.scheme`](/docs/registry/attributes/url.md) | ![Stable](https://img.shields.io/badge/-stable-lightgreen) | `Conditionally Required` If and only if the target URL contains a scheme. | string | The scheme portion of the RPC target URL used when creating the gRPC channel. [6] | `dns`; `xds`; `my-resolver` |
...
| [`server.port`](/docs/registry/attributes/server.md) | ![Stable](https://img.shields.io/badge/-stable-lightgreen) | `Recommended` [10] | int | The RPC server port portion of the target URL used when creating the gRPC channel. [11] | `50051` |
...
**[5] `server.address`:** The address SHOULD be parsed from the target URL according to the
[gRPC Name Resolution specification](https://grpc.github.io/grpc/core/md_doc_naming.html) depending on the scheme used.
...
If instrumentation cannot determine a single address from the target URL
(for example, when multiple IP addresses are provided),
it SHOULD NOT set this attribute.

Examples:

- given the target URL `grpc.io::50051`, the attributes would be:
  - `server.address`: `grpc.io`
  - `server.port`: `50051`
  - `url.scheme`: not set
- given the target URL `xds:///wallet.grpcwallet.io`, the attributes would be:
  - `server.address`: `wallet.grpcwallet.io`
  - `server.port`: not set
  - `url.scheme`: `xds`
- given the target URL `unix:///run/containerd/containerd.sock`, the attributes would be:
  - `server.address`: `/run/containerd/containerd.sock`
  - `server.port`: not set
  - `url.scheme`: `unix`

**[6] `url.scheme`:** See the note for `server.address` for details on how to parse the port from the target URL.
...
**[10] `server.port`:** If and only if `server.address` is set and the target URL contains a port.

**[11] `server.port`:** See the note for `server.address` for details on how to parse the port from the target URL.
