...
- Given the target string `dns://1.2.3.4/grpc.io:50051`, expected attributes:
  - `server.address`: `"grpc.io"`
  - `server.port`: `50051`
  - `grpc.target`: `"dns://1.2.3.4/grpc.io:50051"`
...
**[5] `grpc.target`:** If the channel was created with a target string and it contains information that's not otherwise captured in `server.address` and `server.port`.
