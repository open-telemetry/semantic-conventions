...
| [`server.port`](/docs/registry/attributes/server.md) | ![Stable](https://img.shields.io/badge/-stable-lightgreen) | `Recommended` [10] | int | Server port number. [11] | `50051` |
...
**[10] `server.port`:** If and only if the port is available and `server.address` is set.

**[11] `server.port`:** See the `server.address` for details on parsing the target string.
